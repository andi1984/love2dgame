...
local function startRace(trackConfig)
    ...
    -- NPC cars (stagger behind player with enough spacing to avoid overlap)
    for i, profile in ipairs(npcProfiles.list) do
        local npcCar = Car.new(track, {
            ...
            startOffset = -i * 0.04,
            lateralOffset = (i % 2 == 1) and 12 or -12,
        })
...
function love.update(dt)
    ...
    if state.is("racing") then
        ...
        -- (skip for first 2 seconds so grid-start spacing settles)
        -- ----------------------------------------------------------------
        if game.timer >= 2.0 then
